...
public record LinkUpdate(
        ...
        /** Уникальный идентификатор обновления ссылки. Должен быть больше или равен 1. */
        @NotNull(message = "ID is required") @Min(value = Constant.MIN_ID, message = "ID can't be less than 1") Long id,

        /** URL для обновления ссылки. Не должен быть пустым и должен содержать не более 2048 символов. */
        @NotNull(message = "URL is required")
                @NotEmpty(message = "URL cannot be empty")
                @Size(max = Constant.MAX_URL_LENGTH, message = "URL must be less than 2048 characters")
                String url,

        /** Описание обновления ссылки. Не должно быть пустым и должно содержать не более 500 символов. */
        @NotNull(message = "Description is required")
                @NotEmpty(message = "Description cannot be empty")
                @Size(max = Constant.MAX_DESCRIPTION_LENGTH, message = "Description must be less than 500 characters")
                String description,

        /** Список идентификаторов чатов Telegram для уведомлений. Не должен быть пустым. */
