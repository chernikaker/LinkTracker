...
public class OrmSubscriptionMapper {
    ...
    public Subscription mapFromOrm(OrmSubscription ormSubscr) {
        ...
        List<Filter> filters = ormSubscr.filters().stream().map(OrmFilterMapper::mapFromOrm).toList();
        return new Subscription(
            OrmUserMapper.mapFromOrm(ormSubscr.user()),
            OrmLinkMapper.mapFromOrm(ormSubscr.link()),
            tags,
            filters
        );
    }
